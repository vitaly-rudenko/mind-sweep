    ...
    <Drawer open={open} onOpenChange={(open) => !open && onClose()} dismissible={false}>
      <DrawerContent>
        <div className='mx-auto w-full max-w-sm'>
          <DrawerHeader>
            <DrawerTitle>Add Integration</DrawerTitle>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)}>
              <div className='px-4 flex flex-col gap-2'>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className='flex flex-col'>
                      <Select onValueChange={field.onChange} defaultValue={field.value} open={selectOpen} onOpenChange={setSelectOpen}>
                        <SelectTrigger>
                ...
                {$integrationType === 'notion' && (
                  <FormField
                    control={form.control}
                    name='metadata.integrationSecret'
                    render={({ field }) => (
                      <FormItem>
                        <Input {...field} placeholder='Integration Secret' />
                      </FormItem>
                    )}
                  />
                )}
